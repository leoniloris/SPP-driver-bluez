...
#define UUID_GAP 0x1100
#define UUID_GATT 0x1101
...
struct server {
  ...
  uint16_t serial_attibute_handle;
  uint16_t tx_attribute_handle;
  bool visibility;
...
static void tx_descriptor_read_cb(struct gatt_db_attribute *attrib,
                                  unsigned int id, uint16_t offset,
                                  uint8_t opcode, struct bt_att *att,
                                  void *user_data) {
  // struct server *server = user_data;
...
static void tx_descriptor_write_cb(struct gatt_db_attribute *attrib,
                                   unsigned int id, uint16_t offset,
                                   const uint8_t *value, size_t len,
                                   uint8_t opcode, struct bt_att *att,
                                   void *user_data) {
  ...
  if (value[0] == 0x00)
    ...
    if (server->tx_notification_enabled) {
      PRLOG("HR Measurement Already Enabled\n");
      goto done;
    }

    server->tx_notification_enabled = true;
  } else
    ecode = 0x80;

  PRLOG("HR: Measurement Enabled: %s\n",
        server->tx_notification_enabled ? "true" : "false");

done:
  gatt_db_attribute_write_result(attrib, id, ecode);
}

static void rx_characteristic_cb(struct gatt_db_attribute *attrib,
                                 unsigned int id, uint16_t offset,
                                 const uint8_t *value, size_t len,
                                 uint8_t opcode, struct bt_att *att,
                                 void *user_data) {
  ...
  // Custom rx callback or post event goes here.
done:
  gatt_db_attribute_write_result(attrib, id, ecode);
}
...
static struct option main_options[] = {{"index", 1, 0, 'i'},
                                       {"mtu", 1, 0, 'm'},
...
static struct option notify_options[] = {{"indicate", 0, 0, 'i'}, {}};
...
int main(int argc, char *argv[]) {
  ...
  server = server_create(fd, mtu, visibility);
  if (!server) {
    close(fd);
    return EXIT_FAILURE;
  }

  // if (mainloop_add_fd(fileno(stdin), EPOLLIN | EPOLLRDHUP | EPOLLHUP | EPOLLERR,
